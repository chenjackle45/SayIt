...
- Windows 錄製自訂觸發鍵時，若 SayIt 設定視窗還在前景，按什麼都錄不到、10 秒後逾時，要先點到別的視窗再按才行（#78、#70）：回報者的一鍵診斷紀錄顯示全域鍵盤 hook 在自家視窗有焦點時完全收不到事件。錄製期間改由設定頁自己的鍵盤事件補上（只在 Windows 啟用），規則與原本一致：先放開的修飾鍵不算、錄製前按住的不算、兩顆修飾鍵一起按放可錄成組合鍵；視窗失焦或遇到輸入法／AltGr 這類判不出鍵碼的按鍵，就交回原本的 hook 處理。錄製中的按鍵不會打進頁面欄位

## [0.14.0] - 2026-09-12

### Fixed

- Windows 自訂觸發鍵錄製（#30）：根因是錄製時判斷「哪些修飾鍵按著」靠 `GetKeyState`，在鍵盤 hook 執行緒上讀到的是上一個事件之前的狀態——按下讀到「沒按」、放開讀到「還按著」——所以 Ctrl＋C 只錄到 C、單獨一顆右 Ctrl／右 Alt 永遠逾時。改成兩平台各自從鍵盤事件維護「目前按著的實體修飾鍵」集合（鍵碼、左右分開），錄製與觸發都只看事件本身，`GetKeyState` 整個拿掉；macOS 以系統修飾旗標校正、失同步時家族全放開即自癒。回報者附的一鍵診斷紀錄是這次的證據
- 錄製組合鍵時，先放開的修飾鍵不會再被算進去（例：Ctrl↓ Alt↓ Ctrl↑ K↓ 錄成 Alt＋K）；錄製前就按住的鍵不算；錄製中畫面提示「請先放開所有按鍵」

### Added

- 純修飾鍵組合可以當觸發鍵（#30）：例如 Windows 的右 Alt＋右 Ctrl、macOS 的右 Option＋右 Control，錄製時同時按下兩顆再放開即可；左右鍵分開認、按下順序不拘、兩顆按齊開始、放開任一顆結束，Hold／Toggle／雙擊切換模式都適用；Fn 也可以和其他修飾鍵組合

### Changed

- 每次更新後首次開啟 Dashboard 的「更新摘要」彈窗換成 v0.14.0 內容（五語系，兩條）
- **降版注意**：純修飾鍵組合是新的設定型態，舊版不認得；若要退回 v0.13.x，請先在設定頁切回預設觸發鍵再降版
- ElevenLabs 轉錄服務順延至 v0.15

## [0.13.1] - 2026-09-11

### Fixed

- Windows 自訂觸發鍵錄製：前端改成先掛好事件監聽再叫 Rust 進入錄製模式，修掉一個會漏接「已錄到」事件的空窗；註冊期間取消不會再偷偷啟動錄製（#30）

### Added

- 自訂觸發鍵錄製失敗（逾時、無法開始、被拒）後，快捷鍵卡片出現「回報到 GitHub」與「複製診斷資訊」：一鍵整理版本、平台、觸發鍵設定與錄製期間的程式記錄，開已填好的 issue 頁或複製貼給開發者。內容只含錄製期間的按鍵代碼、設定與程式階段記錄，不含任何轉錄內容；不需要先開「除錯記錄」（#30）
- Windows 錄製路徑加診斷日誌（hook 收到的鍵只在錄製中記、鎖競爭只記階段），供上述回報使用

### Changed

- 每次更新後首次開啟 Dashboard 的「更新摘要」彈窗換成 v0.13.1 內容（五語系，一條）
- 開發規則：移除「UI 實作前先出 design.pen 設計稿」，改為小改動直接做、截圖驗收；大改版走拍板 demo

## [0.13.0] - 2026-09-11

### Added

- 轉錄語言新增「廣東話」（#74）：語音辨識送 `yue`（Groq whisper-large-v3 實測接受、粵語用字更準），內建「精簡」「積極」整理模式加粵語版，保留粵語用字與句式、不再改寫成書面語；簡→繁轉換對粵語同樣套用。自訂 prompt 內容不動；`whisper-large-v3-turbo` 對粵語會偏普通話，建議用預設模型
- 模型清單標示免費專案可用性（#71）：OpenAI 兩款與 Claude Haiku 4.5 標「需付費」、Gemini 3.5 Flash 標「免費額度少」，模型說明各多一句提示，避免免費專案選到就 429

### Changed

- 設定頁收成「語音轉錄」「文字整理」兩張功能卡：Groq API Key 與轉錄語言跟著語音轉錄走，LLM 服務／金鑰／模型收進文字整理；頁首獨立金鑰卡拿掉、應用程式設定不再有轉錄語言列（拍板 demo：`docs/demos/elevenlabs-settings/settings-redesign.html` 設計 A）
- 每次更新後首次開啟 Dashboard 的「更新摘要」彈窗換成 v0.13.0 內容（五語系，三條）

## [0.12.1] - 2026-09-11

### Fixed

- Windows 觸發鍵設為左 Ctrl（Hold 或 Toggle）時，v0.12.0 每次錄音都立刻停止、顯示「未偵測到語音」：v0.12.0 在錄音開始時模擬 Ctrl+C 做選取探測，模擬的 Ctrl↓／Ctrl↑ 被鍵盤 hook 當成觸發鍵動作。現在 SayIt 自己送出的按鍵一律帶記號，hook 直接放行；改鍵工具注入的按鍵不受影響（計畫：`docs/plan-windows-injected-key-guard.md`）

### Added

- 「啟動時隱藏主視窗」設定：手動或開機啟動後只保留選單列（系統列）圖示，從圖示選單或再開一次 SayIt 即可叫出（#76）。新功能隨 patch 版出是例外：已合入 main 且與修復不可分

### Changed

- 每次更新後首次開啟 Dashboard 的「更新摘要」彈窗換成 v0.12.1 內容（五語系，兩條）

## [0.12.0] - 2026-09-10

### Added

- 主題設定：淺色／深色／跟隨系統（預設跟隨系統），HUD 與 Dashboard 一致套用、跨視窗同步；Windows 由 Rust 讀登錄檔並輪詢廣播系統主題變化（#61，@lettucebo）
- 歷史紀錄每筆可「重新辨識」（用保存的錄音重送轉錄）或「重新整理」（用原文重跑 AI 整理）；重試鎖存於 store、編輯模式紀錄不允許重試以免清掉編輯結果；LLM 空回應時保留既有結果（#60，@lettucebo；#42、#75）
- 可選的除錯檔案日誌，預設關閉：設定頁「除錯記錄（進階）」開關，記錄檔寫入 app_log_dir()/sayit.log、可一鍵開啟資料夾；記錄檔含轉錄文字與 AI 整理結果，關閉開關即清除記錄檔（#58，@lettucebo）
- 字典單檔匯出／匯入：匯出帶版本的 JSON，匯入以詞條（不分大小寫）合併；也接受 .txt（一行一詞）與 .csv（取第一欄）方便從 Typeless 等工具遷移；權重上限 1000、單次上限 5000 筆（#41，@ASTROHSU；#40、#5）

### Fixed

- Windows 自動貼上回歸（#70、#72）：還原 v0.10.0 的選取探測時序，錄音開始時即做剪貼簿探測，避免觸發鍵放開後才送 Ctrl+C 導致瀏覽器貼上失焦。已知代價：Windows 上無選取時會複製整行的編輯器（#73）誤判率回到 v0.10.0 水準
- Sentry 不再收集 console breadcrumb，避免逐字稿內容進入錯誤回報

### Changed

- 每次更新後首次開啟 Dashboard 的「更新摘要」彈窗換成 v0.12.0 內容（五語系）
- IPC 契約表補 `get_os_theme` 指令與 `theme:os-changed` 事件
