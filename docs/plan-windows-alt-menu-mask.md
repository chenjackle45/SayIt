...
> 建立：2026-09-13 · 狀態：codex 計劃閘三輪（R1：2 HIGH 5 MED；R2：1 HIGH 3 MED；R3：1 HIGH 1 MED，設計面已收斂、剩驗收寫法；全收，見文末）→ 計劃閘通過 · 🔴 **2026-09-20 起暫緩動工**（見下方「回音結果」） · 排程：v0.14.2（小卡批，#24 另卡）
> 來源：#70 cruelforever 2026-09-11 留言「在其他程式上使用右 Alt 會跳出，辨識完不會貼到應該在的對話框，手動 Ctrl+V 可以」；2026-09-13 已回問「放開右 Alt 後選單列是否反白」（issuecomment-5652908237）。

## 回音結果（2026-09-20 補）

cruelforever 2026-09-14 回覆五題：v0.14.1、模式 **Hold**、Edge 正常但 Claude Code 對話框跳選單、輸入法為華碩智慧輸入法、**重貼前「必須再點一下對話框」**。

**動工前提未達成**，兩個理由：

1. **通過判準的對照實驗沒做成**。判準要的是「不點輸入框、不按 Esc，直接手動 Ctrl+V → 沒貼進」；回報者做的是先點對話框再貼，正是判準明文排除的做法。成因因此仍停在「可信候選」，沒有升級。模式是 Hold 並不構成問題——上表第三列與「三問」都已涵蓋。
2. **回報者已自行繞過**：改用「右 Alt＋.」組合鍵後「可以正常輸入而不會跳出對話框」，他自評「這個就沒什麼問題了」。痛點強度下降。

**併案要求**：2026-09-14 新開的 #79（Hold＋Alt／Shift 選取後不進編輯模式）與 #80（Hold＋Alt 觸發先多打一個 `C`）指向 `build_ctrl_chord_inputs()` 的另一個問題——注入 Ctrl+C 時使用者的實體修飾鍵仍按著，目標程式實收 `Alt+Ctrl+C`。那是與本計畫**獨立的問題**（按鍵事件仍有送出，lone-Alt 判定仍被打斷），但兩者共用同一段探測時序，而 `.claude/rules/windows.md` 明文「改動這段時序等於重開 #70／#72」。**三串必須併案設計，本計畫不得單獨動工。**
